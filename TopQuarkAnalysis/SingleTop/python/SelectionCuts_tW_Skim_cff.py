...
#For muLoose Rebeca also has isTrackerMuon requirement in... we put the variable in EDMtuple to study it, since it is a deviation from the dilepton reference selection
#We also added the other variables on which TightMuons are cut, so that we can reproduce Rebeca's looseMu selection (that is Tight with relaxed Pt, |Eta| and RelIso).
...
#Taking away the EB/EE transition region cut since not used by Rebeca nor by dilepton.
#check about gsfElectron
eleTightCut = cms.string("et>20  & abs(eta)<2.5  ") #& ( abs(superCluster.eta)> 1.5660 || abs(superCluster.eta)<1.4442)")
#Rebeca relaxed cut on numberOfMatches (muon hits) and PixelLayers. We include them both in the EDMtuple, even though the number of muon hits should be automatically >0 with the globalMuonPromptTight... let's check.
#PFRelIso cut done offline also
muTightCut = cms.string("pt > 20 & isGlobalMuon & isTrackerMuon & abs(eta) < 2.4 & muonID('GlobalMuonPromptTight') > 0 & innerTrack.numberOfValidHits > 10 ") # #&& numberOfMatches() > 0  && innerTrack.hitPattern.pixelLayersWithMeasurement() >=1 ")

#Jet definition
#Took out for now the standard cut:
#& ((abs(eta())>2.4) || ( chargedHadronEnergyFraction() > 0 & chargedMultiplicity()>0))
#and put the variables in the EDMTuple to check offline.
jetLooseCut = cms.string("numberOfDaughters()>1 & pt()> 20 & abs(eta())<5 & neutralEmEnergyFraction() < 0.99 & neutralHadronEnergyFraction() < 0.99 & chargedEmEnergyFraction()<0.99") 
