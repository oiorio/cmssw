/*
*\Author:  O.Iorio, M.Merola
*
*
*
*\version  $Id: SingleTopSystematicsTreesDumper_SChannelMu.cc,v 1.12.2.18.2.16.2.5 2013/04/19 15:09:54 oiorio Exp $
*/
...
#include "TopQuarkAnalysis/SingleTop/interface/SingleTopSystematicsTreesDumper_SChannelMu.h"
...
SingleTopSystematicsTreesDumper_SChannelMu::SingleTopSystematicsTreesDumper_SChannelMu(const edm::ParameterSet &iConfig)
{
...
void SingleTopSystematicsTreesDumper_SChannelMu::initBranchVars()
{
#define INT_NAN 9999
...
void SingleTopSystematicsTreesDumper_SChannelMu::analyze(const Event &iEvent, const EventSetup &iSetup)
{
	...
	topPtReweightMCTruth =1.;
		...
		//		cout << "jet i " <<  i << endl;
                ...
                if (passesPtCut)
		  {
		    ...
		    /*		    
		    if (syst == "noSyst")
		      cout << "for syst: " << syst << " " << jets[nJets - 1].pt() << endl;
		    if (syst == "JERUp")
		      cout << "for syst: " << syst << " " << jets[nJets - 1].pt() << endl;
		    if ( syst == "JERDown")
		      cout << "for syst: " << syst << " " << jets[nJets - 1].pt() << endl;
	...
	//if (thirdPt > secondPt ) cout << "  sanity check: Pt3 > Pt2 at event" << eventTree << " njets "<< nJets <<endl;
	  ...
	  cout << "BEFORE nJets: " << nJets << "  nJetsTight: " << nJetsTight << endl;
        */
        ...
        if (secondPt < ptCutTight) // PROBLEM TO BE SOLVED IN THE NEXT ITERATION AFFECTING NJ=0,1
          nJets = nJetsTight;
	...
	/*	
	if (syst == "JERUp")
	  cout << "for syst: " << syst << "  nJets: " << nJets << "   nJetsTight: "  << nJetsTight << "   " << jets[0].pt() << "  "  << jets[1].pt() << "  " << jets[2].pt() << "  " << jets[3].pt() << endl;
	if (syst == "noSyst")
          cout << "for syst: " << syst << " nJets: " << nJets << "   nJetsTight: "  << nJetsTight   << "   " << jets[0].pt() << "  "  << jets[1].pt() << "  " << jets[2].pt() << "  " << jets[3].pt() << endl;
	if ( syst == "JERDown")
          cout << "for syst: " << syst << "  nJets: " << nJets << "   nJetsTight: "  << nJetsTight   << "   " << jets[0].pt() << "  "  << jets[1].pt() << "  " << jets[2].pt() << "  " << jets[3].pt() << endl;
	*/
	    ...
	    math::XYZTLorentzVector neutrino = NuMomentum(leptonPFour.px(),leptonPFour.py(),leptonPFour.pz(),lepton_Pt,leptonPFour.E(),metPx,metPy);
            ...
            if (B == 0 || B == 2 || B == 3 || B == 5){   // B == 3,5 for 0T,2T_QCD sample
	      if (B==0 || B==3){
		...
		bjets[1] = jets[lowBTagTreePosition];
	      ...
	      /*	
		if (syst == "JERUp")
		cout << "for syst: " << syst << "   correction:   " << smear << "  "  << energyCorr <<"  "  << ptCorr << "  " << endl;
		if (syst == "noSyst")
		cout << "for syst: " << syst << "   correction:   " << smear << "  "  << energyCorr <<"  "  << ptCorr << "  " << endl;
		if ( syst == "JERDown")
		cout << "for syst: " << syst << "   correction:   " << smear << "  "  << energyCorr <<"  "  << ptCorr << "  " << endl;
		...
		if (B==2){
		if (syst == "JERUp")
		cout << "for syst: " << syst << "  nJets: "  << nJets << "  "  << bJet1Pt << "  " << bJet2Pt << endl;
		if (syst == "noSyst")
		cout << "for syst: " << syst << "  nJets: "  << nJets << "  "  << bJet1Pt << "  " << bJet2Pt << endl;
		if ( syst == "JERDown")
		cout << "for syst: " << syst << "  nJets: "  << nJets << "  "  << bJet1Pt << "  " << bJet2Pt << endl;
	    ...
	    math::PtEtaPhiELorentzVector top_best =(abs(topMassTree-172.5)<abs(topMassTree_fJet-172.5))? top: top_fJet;
            if(top_best==top) {cosTree_best=cosTree;  bestJetFlavourTree = bJetFlavourTree; topMassTree_best =topMassTree;}
            else {cosTree_best=cosTree_bJet; bestJetFlavourTree = fJetFlavourTree; topMassTree_best =topMassTree_fJet; }
...
float SingleTopSystematicsTreesDumper_SChannelMu::cosThetaLJ(math::PtEtaPhiELorentzVector lepton, math::PtEtaPhiELorentzVector jet, math::PtEtaPhiELorentzVector top)
{
...
float SingleTopSystematicsTreesDumper_SChannelMu::cosTheta_bl_bjet(math::PtEtaPhiELorentzVector jet, math::PtEtaPhiELorentzVector Wint, int chargeTree)
{
...
math::PtEtaPhiELorentzVector SingleTopSystematicsTreesDumper_SChannelMu::Wint4Momentum(math::PtEtaPhiELorentzVector lepton, math::PtEtaPhiELorentzVector jet1, math::PtEtaPhiELorentzVector jet2, float metPx, float metPy){
...
math::PtEtaPhiELorentzVector SingleTopSystematicsTreesDumper_SChannelMu::Wint4Momentum(math::PtEtaPhiELorentzVector lepton, math::PtEtaPhiELorentzVector jet1, math::PtEtaPhiELorentzVector jet2, math::PtEtaPhiELorentzVector jet3, float metPx, float metPy){
...
math::PtEtaPhiELorentzVector SingleTopSystematicsTreesDumper_SChannelMu::Wint4Momentum(float leptonPx, float leptonPy, float leptonPz, float leptonE, float jet1Px, float jet1Py,float jet1Pz, float jet1E, float jet2Px, float jet2Py, float jet2Pz, float jet2E, float metPx, float metPy){
...
float SingleTopSystematicsTreesDumper_SChannelMu::cosTheta_eta_bl(math::PtEtaPhiELorentzVector lepton, math::PtEtaPhiELorentzVector jet, math::PtEtaPhiELorentzVector top)
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::topMtw(math::PtEtaPhiELorentzVector lepton, math::PtEtaPhiELorentzVector jet, float metPx, float metPy)
{
...
math::PtEtaPhiELorentzVector SingleTopSystematicsTreesDumper_SChannelMu::top4Momentum(math::PtEtaPhiELorentzVector lepton, math::PtEtaPhiELorentzVector jet, float metPx, float metPy)
{
...
math::PtEtaPhiELorentzVector SingleTopSystematicsTreesDumper_SChannelMu::top4Momentum(float leptonPx, float leptonPy, float leptonPz, float leptonE, float jetPx, float jetPy, float jetPz, float jetE, float metPx, float metPy)
{
    float lepton_Pt = sqrt( (leptonPx * leptonPx) +  (leptonPy * leptonPy) );
...
math::XYZTLorentzVector SingleTopSystematicsTreesDumper_SChannelMu::NuMomentum(float leptonPx, float leptonPy, float leptonPz, float leptonPt, float leptonE, float metPx, float metPy )
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::jetUncertainty(double eta, double ptCorr, int flavour)
{
...
float SingleTopSystematicsTreesDumper_SChannelMu::muonHLTEff(float eta, string period)
{
    float eff = 0.87;
...
double SingleTopSystematicsTreesDumper_SChannelMu::topPtReweighting(string chanrew, double pt){
  if((channel == "TTBar") || 
...
void SingleTopSystematicsTreesDumper_SChannelMu::endJob()
{
...
void SingleTopSystematicsTreesDumper_SChannelMu::resetWeightsDoubles()
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::BTagSFNew(double pt, string algo)
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::BTagSFErrNew(double pt, string algo)
{
    if (algo == "TCHPT")
    {
...
double SingleTopSystematicsTreesDumper_SChannelMu::MisTagSFNew(double pt, double eta, string algo)
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::MisTagSFErrNewUp(double pt, double eta, string algo)
{
    double x = pt;
...
double SingleTopSystematicsTreesDumper_SChannelMu::MisTagSFErrNewDown(double pt, double eta, string algo)
{
    double x = pt;
...
double SingleTopSystematicsTreesDumper_SChannelMu::EFFMapNew(double btag, string algo)
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::EFFMap(string algo )
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::EFFMap(string algo, string channel )
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::EFFErrMap(string algo )
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::turnOnWeight (std::vector<double> probabilities, int njets_req = 1)
{
    double prob = 0;
...
int SingleTopSystematicsTreesDumper_SChannelMu::eventFlavour(string ch, int nb, int nc, int nl)
{
...
bool SingleTopSystematicsTreesDumper_SChannelMu::flavourFilter(string ch, int nb, int nc, int nl)
{
...
/*double SingleTopSystematicsTreesDumper_SChannelMu::jetprob(double pt, double btag){
...
double SingleTopSystematicsTreesDumper_SChannelMu::jetprobpt(double pt)
{
    double prob = 0.982 * exp(-30.6 * exp(-0.151 * pt)); //PT turnOn
    return prob;
}

double SingleTopSystematicsTreesDumper_SChannelMu::jetprobbtag(double btag)
{
    double prob = 0.844 * exp((-6.72 * exp(-0.720 * btag))); //BTag turnOn
    return prob;
}

double SingleTopSystematicsTreesDumper_SChannelMu::turnOnProbs(string syst, int n)
{
...
void SingleTopSystematicsTreesDumper_SChannelMu::pushJetProbs(double pt, double btag, double eta)
{
...
void SingleTopSystematicsTreesDumper_SChannelMu::InitializeTurnOnReWeight(string rootFile = "CentralJet30BTagIP_2ndSF_mu.root")
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::turnOnReWeight (double preWeight, double pt, double tchpt)
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::jetprob(double pt, double btag, double eta, string syst)
{
    double prob = 1.;
...
double SingleTopSystematicsTreesDumper_SChannelMu::jetprobold(double pt, double btag, double eta, string syst)
{
    double prob = 1.;
...
double SingleTopSystematicsTreesDumper_SChannelMu::bTagSF(int B)
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::bTagSF(int B, string syst)
{
  if (doBTagSF_ == false )return 1.0;
...
double SingleTopSystematicsTreesDumper_SChannelMu::pileUpSF(string syst)
{
...
double SingleTopSystematicsTreesDumper_SChannelMu::resolSF(double eta, string syst)
{
    double fac = 0.;
    if (syst == "JERUp")fac = 1.;
...
//BTag weighter
bool SingleTopSystematicsTreesDumper_SChannelMu::BTagWeight::filter(int t)
{
    return (t >= minTags && t <= maxTags);
}

float SingleTopSystematicsTreesDumper_SChannelMu::BTagWeight::weight(vector<JetInfo> jets, int tags)
{
    if (!filter(tags))
    {
...
float SingleTopSystematicsTreesDumper_SChannelMu::BTagWeight::weightWithVeto(vector<JetInfo> jetsTags, int tags, vector<JetInfo> jetsVetoes, int vetoes)
...
void SingleTopSystematicsTreesDumper_SChannelMu::muonHLTSF(float etaMu, float ptMu){
...
DEFINE_FWK_MODULE(SingleTopSystematicsTreesDumper_SChannelMu);
