import FWCore.ParameterSet.Config as cms

process = cms.Process("SingleTopTG")

process.maxEvents = cms.untracked.PSet( input = cms.untracked.int32(100) )
#process.maxEvents = cms.untracked.PSet( input = cms.untracked.int32(-1) )
process.source = cms.Source ("PoolSource",
                             fileNames = cms.untracked.vstring (
    #'rfio:/castor/cern.ch/user/o/oiorio/SingleTop/SingleTop_tChan/TChanSampleTChanMu_9_1_S3X.root',
#    'file:/tmp/oiorio/00012F91-72E5-DF11-A763-00261834B5F1.root'
#    'file:/tmp/oiorio/EMEnriched_30to80Merged.root'
#    'file:/tmp/oiorio/QCDMuMerged.root'
#    'file:/tmp/oiorio/TChannelMerged.root'
"file:/afs/cern.ch/work/o/oiorio/public/xFrancescoFab/T_t-channel_Synch.root"
#"file:/tmp/oiorio/edmntuple_TbarChannel_9_1_Qbe.root"
),
skipBadFiles = cms.untracked.bool(True),
duplicateCheckMode = cms.untracked.string('noDuplicateCheck')
)



## Load additional RECO config
process.load("Configuration.StandardSequences.Geometry_cff")
process.load("Configuration.StandardSequences.FrontierConditions_GlobalTag_cff")

#process.GlobalTag.globaltag = cms.string('START52_V9::All')

process.GlobalTag.globaltag = cms.string('START53_V11::All')
process.HLTDumper1 = cms.EDAnalyzer('HLTInspect',
                                   HLTriggerResults = cms.InputTag("TriggerResults","","HLT")
#                                   HLTriggerResults = cms.InputTag("TriggerResults","","SingleTop")
                                   )

process.HLTDumper2 = cms.EDAnalyzer('HLTInspect',
                                   HLTriggerResults = cms.InputTag("TriggerResults","","SingleTop")
#                                   HLTriggerResults = cms.InputTag("TriggerResults","","SingleTop")
                                   )

process.HLTDumper3 = cms.EDAnalyzer('HLTInspect',
                                   HLTriggerResults = cms.InputTag("TriggerResults","","")
#                                   HLTriggerResults = cms.InputTag("TriggerResults","","SingleTop")
                                   )

process.HLTDumper4 = cms.EDAnalyzer('HLTInspect',
                                   HLTriggerResults = cms.InputTag("TriggerResults","","HLT")
#                                   HLTriggerResults = cms.InputTag("TriggerResults","","SingleTop")
                                   )

#process.skimwall = cms.OutputModule("PoolOutputModule",
#                                    fileName = cms.untracked.string('TChannelMerged.root'),
#                                    outputCommands = cms.untracked.vstring(    'keep *',   )
#)

process.path = cms.Path(
    process.HLTDumper1
#    process.HLTDumper2+
#    process.HLTDumper3+
#    process.HLTDumper4
    )
#process.source.fileNames = TChannel_ntuple
#process.skimwall.fileName = "/tmp/oiorio/TChannelMerged.root"
#process.skimwall.fileName = "/castor/cern.ch/user/o/oiorio/SingleTop/2011/MC2011/NewNtuples/Merged/TChannelMerged.root"

#Save the skims
#process.outpath = cms.EndPath( process.skimlight + process.skimwcc + process.skimwbb )
#process.outpath = cms.EndPath( process.skimwall)
