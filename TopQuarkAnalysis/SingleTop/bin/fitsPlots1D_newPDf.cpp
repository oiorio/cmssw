...
//  channelsplusqcd.push_back("QCDMu");  channelsplusqcd.push_back("QCDEle");
  ...
  observables.push_back("PDFWeight");
  minobs["PDFWeight"]=0;
  maxobs["PDFWeight"]=.5;
  nbins["PDFWeight"]=1.5;
  nbinseleeta["PDFWeight"]=1000;
  ...
  for (int lep = 0; lep <2;++lep){ 
	...
	for(int ch = 0; ch < 2; ++ch){
	  ...
	  for (int ud = 0;ud<n_syst_extremes;++ud){
		...
		string pathttsample = "Trees"+lepton+"/"+channel+"_NJets_"+systUpDownTree;
		treesNJets[channel][systUpDownTree][lepton] = (TTree*)f.Get((pathttsample).c_str());
		...
		if(useSystPDF){
		  ...
		  TH1D * tmp = new TH1D("tmp","tmp",1000,0.5,1.5);
		  treesNJets[channel][systUpDownTree][lepton]->Project("tmp",pdf_names[p-1]);
