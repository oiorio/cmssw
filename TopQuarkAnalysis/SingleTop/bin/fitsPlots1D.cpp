...
int main(int argc,  char* argv[]){
  ...
  map<string, map< string, map < string, TTree *> > > treesNJets;
  ...
  */
  ...
  if(useSystModel){
    ...
    systematics_total.push_back("Mass"); 
  ...
  if(useSystModel){
    ...
    channels.push_back("TChannel_MassDown");
    channels.push_back("TChannel_MassUp");

    channels.push_back("TbarChannel_MassDown");
    channels.push_back("TbarChannel_MassUp");

    channels.push_back("TTBar_MassDown");
    channels.push_back("TTBar_MassUp");
...
//  channelsplusqcd.push_back("QCDMu");  channelsplusqcd.push_back("QCDEle");
  ...
  if(useSystPDF){
    for(int p =1;p<=npdfstorun;++p){
      if (!(p >= MinPDF && p <=MaxPDF))continue;  
      observables.push_back("PDFWeightTotal"+pdf_names[p-1]);
      minobs["PDFWeightTotal"+pdf_names[p-1]]=0.50;
      maxobs["PDFWeightTotal"+pdf_names[p-1]]=1.50;
      nbins["PDFWeightTotal"+pdf_names[p-1]]=1000;
      nbinseleeta["PDFWeightTotal"+pdf_names[p-1]]=1000;
    }
  }
  ...
  TH1D * eta_TopSR1_From_3J2T = new TH1D("eta_TopSR1_From_3J2T","eta_TopSR1_From_3J2T",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);

  TH1D * eta_TopSR2_From_3J2T = new TH1D("eta_TopSR2_From_3J2T","eta_TopSR2_From_3J2T",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);


  TH1D * eta_TopSR1_From_3J2T_Full = new TH1D("eta_TopSR1_From_3J2T_Full","eta_TopSR1_From_3J2T",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);

  TH1D * eta_TopSR2_From_3J2T_Full = new TH1D("eta_TopSR2_From_3J2T_Full","eta_TopSR2_From_3J2T",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);

  TH1D * eta_TopTmpRemodelSR1_From_3J2T = new TH1D("eta_TopTmpRemodelSR1_From_3J2T","eta_TopTmpRemodelSR1_From_3J2T",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);


  TH1D * eta_TopTmpSR1_From_3J2T = new TH1D("eta_TopTmpSR1_From_3J2T","eta_TopTmSR1_From_3J2T",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);

  TH1D * eta_TopTmpSR2_From_3J2T = new TH1D("eta_TopTmpSR2_From_3J2T","eta_TopTmpSR2_From_3J2T",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);



  TH1D * eta_EWKTopSR1_From_3J2T = new TH1D("etaEWKTopSR1_From_3J2T","etaEWKTopSR1_From_3J2T",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);

  TH1D * eta_EWKTopSR2_From_3J2T = new TH1D("etaEWKTopSR2_From_3J2T","etaEWKTopSR2_From_3J2T",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);

  TH1D * eta_EWKTopSR1_From_3J2T_Full = new TH1D("etaEWKTopSR1_From_3J2T_Full","etaEWKTopSR1_From_3J2T_Full",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);

  TH1D * eta_EWKTopSR2_From_3J2T_Full = new TH1D("etaEWKTopSR2_From_3J2T_Full","etaEWKTopSR2_From_3J2T_Full",
					   nbins["eta"],minobs["eta"],maxobs["eta"]);
  ...
  for(std::vector<string>::const_iterator it = allchannels.begin(); it != allchannels.end(); ++it){
	...
	for(std::vector<string>::const_iterator it_o = observables.begin(); it_o != observables.end(); ++it_o){
	  ...
	  for( int d =0; d< 2 ;++d){
	    ...
	    //CHANGEHERE
	    bool isPDFObservable = false;
	    bool isPDFSysts = false;
	    for(int p =1;p<=npdfstorun;++p){
	      if (!(p >= MinPDF && p <=MaxPDF))continue;  
	      if(syst == pdf_names[p-1]) isPDFSysts = true;
	      if(observable == "PDFWeightTotal"+pdf_names[p-1]) isPDFObservable = true;
	      
	    }
	    bool arePDFSystObservablesEquals=true ;
	    if (isPDFObservable && isPDFSysts){
	      arePDFSystObservablesEquals=false ;
	      if(observable=="PDFWeightTotal"+syst)arePDFSystObservablesEquals=true;
	    }
	    if (!arePDFSystObservablesEquals) continue;
  ...
  for (int lep = 0; lep <2;++lep){ 
	...
	for(int ch = 0; ch < 2; ++ch){
	  ...
	  for (int ud = 0;ud<n_syst_extremes;++ud){
		...
		(syst == "Mass"&& channel != "TTBar_MassDown" && channel != "TTBar_MassUp" &&
		 channel != "TChannel_MassDown" && channel != "TChannel_MassUp" &&
		 channel != "TbarChannel_MassDown" && channel != "TbarChannel_MassUp")  ||
	    ...
	    if(syst=="Mass"){
	      if(channel == "TChannel" || channel == "TbarChannel" || channel == "TTBar") continue;
	      if((systUpDown == "MassUp") && (channel == "TChannel_MassUp" || channel == "TbarChannel_MassUp" || channel == "TTBar_MassUp")){
		systUpDownTree = "noSyst";
	      }
	      else if((systUpDown == "MassDown") && (channel == "TChannel_MassDown" || channel == "TbarChannel_MassDown" || channel == "TTBar_MassDown")){
		systUpDownTree = "noSyst";
	      }
	    }
	    
	    
	    if(systUpDown!= "MassDown"  && (channel == "TChannel_MassDown") )continue;
	    if(systUpDown!= "MassDown"  && (channel == "TbarChannel_MassDown") )continue;
	    if(systUpDown!= "MassDown"  && (channel == "TTBar_MassDown") )continue;

	    if(systUpDown!= "MassUp"  && (channel == "TChannel_MassUp") )continue;
	    if(systUpDown!= "MassUp"  && (channel == "TbarChannel_MassUp") )continue;
	    if(systUpDown!= "MassUp"  && (channel == "TTBar_MassUp") )continue;
	    ...
	    cout <<" treesnamepath " << path.c_str()<<endl;
	    ...
	    if(useSystPDF && (channel == "TChannel" || channel == "TbarChannel")){
	      
	      treesNJets[channel][systUpDownTree][lepton] = new TTree();
	      string pathnjsample = "Trees"+lepton+"/"+channel+"_NJets_"+systUpDownTree;
	      treesNJets[channel][systUpDownTree][lepton] = (TTree*)f.Get((pathnjsample).c_str());
	      
	      for(int p =1;p<=npdfstorun;++p){
	      	if (!(p >= MinPDF && p <=MaxPDF))continue;  
	      	if(syst == pdf_names[p-1]) treesNJets[channel][systUpDownTree][lepton]->SetBranchAddress(pdf_names[p-1].c_str(),&pdf_weights[p-1]);
		
	      }
	      
	      treesNJets[channel][systUpDownTree][lepton]->SetBranchAddress("charge",&chargeVal);
	      treesNJets[channel][systUpDownTree][lepton]->SetBranchAddress("isQCD",&isQCDNJ);
	      
	      //	    map<string, map< string, map < string, TTree *> > > trees3J2T;
	      cout << " test useSystPDF "<< useSystPDF <<endl; 
	      cout << " test syst PDF "<< endl;
	      if(useSystPDF){
		for(int p =1;p<=npdfstorun;++p){
		  if (!(p >= MinPDF && p <=MaxPDF))continue;  
		  if(syst != pdf_names[p-1]) continue;
		  cout << " test syst "<< endl;
		  
		  int njentries =treesNJets[channel][systUpDownTree][lepton]->GetEntries();
		  if(!usesNoSystTree) for(int e = 0; e < njentries; ++e){
		    treesNJets[channel][systUpDownTree][lepton]->GetEntry(e);
		    cout << " pdfweight value " << pdf_weights[p-1]<< " charge val "<< chargeVal<< " isQCD "<< isQCDNJ <<endl;
		    if(!isQCDNJ){
		      
		      if(chargeVal>0 && charge == "Plus"){
			HistoCollection["PDFWeightTotal"+pdf_names[p-1]][channel][systUpDown][lepton]->Fill(pdf_weights[p-1]);
			HistoCollection["PDFWeightTotal"+pdf_names[p-1]][channel][systUpDown][lepton+charge]->Fill(pdf_weights[p-1]);
			HistoCollection["PDFWeightTotal"+pdf_names[p-1]][channel+"SR1"][systUpDown][lepton]->Fill(pdf_weights[p-1]);
			HistoCollection["PDFWeightTotal"+pdf_names[p-1]][channel+"SR1"][systUpDown][lepton+charge]->Fill(pdf_weights[p-1]);
			HistoCollection["PDFWeightTotal"+pdf_names[p-1]][channel+"SR2"][systUpDown][lepton]->Fill(pdf_weights[p-1]);
			HistoCollection["PDFWeightTotal"+pdf_names[p-1]][channel+"SR2"][systUpDown][lepton+charge]->Fill(pdf_weights[p-1]);
		  ...
		  //		  TH1F * tmp = new TH1F("tmp","tmp",1000,0.5,1.5);
		  //treesNJets[channel][systUpDownTree][lepton]->Project("tmp",(pdf_names[p-1]).c_str());
		  //cout << " entries with non zero pdf " << treesNJets[channel][systUpDownTree][lepton]->GetEntries((pdf_names[p-1]+"!=0").c_str())<<" tmpintegral "<< tmp->Integral()<<endl;
		  
		  cout << " test syst " << systUpDown << " lept " << lepton+charge << 
		    " tot PDFWeightTotal"+pdf_names[p-1] << " int " << HistoCollection["PDFWeightTotal"+pdf_names[p-1]][channel][systUpDown][lepton+charge]->Integral()<<endl;
...
;
		...
		if(!(fJetPt > fJetPtCut && bJetPt > fJetPtCut && fJetRMS < fJetRMSCut && mtwMass<80.399 && bJetRMS <10.025 && fJetRMS > -0.01 && eta < 5.0 )) continue;
		...
		if(chargeVal<0 && charge == "Minus"){  
		  ...
		  if( !(fJetPt > fJetPtCut && bJetPt > fJetPtCut && fJetRMS < fJetRMSCut && mtwMass<80.399&& bJetRMS <0.025 && fJetRMS >-0.001 )) continue;
	    ...
	    //if (channel == "TChannel" && syst == "noSyst")
	    cout << " nentriesnow "<< nentries<< endl;
 ...
 //yst== "BTagTrig1" || syst == "JetTrig1") &&
		...
		if( !(fJetPt > fJetPtCut && bJetPt > fJetPtCut && fJetRMS < fJetRMSCut && mtwMass<80.399 && bJetRMS <0.025 && fJetRMS > -0.01 )) continue;
		...
		if( !(secondJetPt > fJetPtCut && fJetRMS < fJetRMSCut && bJetRMS <10.025 && mtwMass<80.399 &&fJetRMS > -0.01 && eta < 5.0)) continue;
		...
		for(size_t o=0; o<observables.size();++o){
		  ...
		  bool arePDFSystObservablesEquals=true ;
		    if (isPDFObservable && isPDFSysts){ arePDFSystObservablesEquals=false ;
		      if(observable=="PDFWeightTotal"+syst)arePDFSystObservablesEquals=true;
		    }
		  if (!arePDFSystObservablesEquals) continue;
	...
	for (int lep = 0; lep <2;++lep){
	  ...
	  for(std::vector<string>::const_iterator it = channels.begin(); it != channels.end(); ++it){
	    ...
	    bool isSysts = channel == "TChannel_Q2Up" || 
	      ...
	      channel == "TTBar_Q2Up" || 
	      channel == "TChannel_MassDown" || 
	      channel == "TChannel_MassUp" || 
	      channel == "TbarChannel_MassDown" || 
	      channel == "TbarChannel_MassUp" || 
	...
	for(std::vector<string>::const_iterator it_o = observables.begin(); it_o != observables.end(); ++it_o){
	  ...
	  for(int ch = 0; ch < 3; ++ch){
		...
		*/		
		...
		/*		if(systUpDown== "noSyst" && charge == ""){
		  eta_TopTmpSR1_From_3J2T->Add(eta_TopSR1_3J2T_Mu);
		  eta_TopTmpSR1_From_3J2T->Add(eta_TopSR2_3J2T_Mu,-1);
		  
		  eta_TopTmpRemodelSR1_From_3J2T->Add(eta_TTBarSR1_Remodel_Mu);
		  //		  eta_TopTmpRemodelSR2_From_3J2T->Add(eta_TTBarSR2_Remodel_Mu,-1);
		  
		  eta_TopSR1_From_3J2T = remodelLimits( eta_TopTmpRemodelSR1_From_3J2T, 
							eta_TopTmpSR1_From_3J2T,
							eta_TopTmpRemodelSR1_From_3J2T,
							false,"_3J2TSR1_remodel",0,limit);
		  
		  eta_TopTmpSR2_From_3J2T->Add(eta_TopSR2_3J2T_Mu);
		  
		  eta_TopTmpRemodelSR2_From_3J2T->Add(eta_TTBarSR2_Remodel_Mu);
		  		  
		  eta_TopSR2_From_3J2T = remodelLimits( eta_TopTmpRemodelSR2_From_3J2T, 
							eta_TopTmpSR2_From_3J2T,
							HistoCollection["eta"]["Data3J2TSR1"]["noSyst"][lepton+charge],
							false,"_3J2TSR1_remodel",0,limit);
							}*/
	...
	for(std::vector<string>::const_iterator it_sam = samples.begin(); it_sam != samples.end(); ++it_sam){
	  ...
	  for (int lep = 0; lep <2;++lep){
		...
		channelfit = "Top";
	      ...
	      if(channel == "TChannel_MassDown" || channel == "TChannel_MassUp")channelfit = "Signal";
	      if(channel == "TbarChannel_MassDown" || channel == "TbarChannel_MassUp")channelfit = "Signal";
		...
		cout<< " syst : " << syst << " obs  " << observable << " histo PDF "<< HistoCollection["PDFWeightTotal"+syst][channelsum+samplesum][systUpDown][lepton+charge]<< endl;
		...
		if(!isPDFObservable && ( channel == "TChannel" || channel == "TbarChannel") && HistoCollection["PDFWeightTotal"+syst][channelsum+samplesum][systUpDown][lepton+charge]!=0 ){
		  if (HistoCollection["PDFWeightTotal"+syst][channelsum+samplesum][systUpDown][lepton+charge]->Integral() != 0 && false){
		    HistoCollection[observable][channel+samplesum][systUpDown][lepton+charge]->Scale(1./HistoCollection["PDFWeightTotal"+syst][channel+samplesum][systUpDown][lepton+charge]->GetMean());
		  } 
		}   
...
//		  }
		...
		isPDFObservable = false;
		for(int p =1;p<=npdfstorun;++p){
		  if(observable == "PDFWeightTotal"+pdf_names[p-1])isPDFObservable=true;
		}
		if(!isPDFObservable && ( channel == "TChannel" || channel == "TbarChannel") && HistoCollection["PDFWeightTotal"+syst][channelsum+samplesum][systUpDown][lepton]!=0){
		  if (HistoCollection["PDFWeightTotal"+syst][channelsum+samplesum][systUpDown][lepton]->Integral() != 0 && false){		
		    cout << " test renorm pdf "<< syst << " ud "<< systUpDown << " channel " << channel << " mean " << HistoCollection["PDFWeightTotal"+syst][channel+samplesum][systUpDown][lepton]->GetMean()  <<endl; 
		    HistoCollection[observable][channel+samplesum][systUpDown][lepton]->Scale(1./HistoCollection["PDFWeightTotal"+syst][channel+samplesum][systUpDown][lepton]->GetMean());
		  } 
		}   
		if(observable !="PUWeight" && channel != "Data" && channel != "WHF"){
  ...
  //Test for Full vs partial Top etaj'
  //_Full
  eta_TopSR1_From_3J2T_Full->Add( HistoCollection["eta"]["TopSR1"]["noSyst"][leptonCase]);
  eta_TopSR1_From_3J2T_Full->Add( HistoCollection["eta"]["TopSR2"]["noSyst"][leptonCase]);
  eta_TopSR2_From_3J2T_Full->Add( HistoCollection["eta"]["TopSR1"]["noSyst"][leptonCase]);
  eta_TopSR2_From_3J2T_Full->Add( HistoCollection["eta"]["TopSR2"]["noSyst"][leptonCase]);
  double intt=eta_TopSR1_From_3J2T_Full->Integral();
  if(intt!=0) eta_TopSR1_From_3J2T_Full->Scale(HistoCollection["eta"]["TopSR1"]["noSyst"][leptonCase]->Integral()/intt);
  if(intt!=0) eta_TopSR2_From_3J2T_Full->Scale(HistoCollection["eta"]["TopSR2"]["noSyst"][leptonCase]->Integral()/intt);
  
  eta_EWKTopSR2_From_3J2T_Full->Add( HistoCollection["eta"]["EWKTopSR2"]["noSyst"][leptonCase]);
  eta_EWKTopSR2_From_3J2T_Full->Add( HistoCollection["eta"]["TopDDSR2"]["noSyst"][leptonCase]);
  eta_EWKTopSR2_From_3J2T_Full->Add(  eta_TopSR2_From_3J2T_Full , -1);

  eta_EWKTopSR1_From_3J2T_Full->Add(eta_EWKTopSR2_From_3J2T_Full);  
  eta_EWKTopSR1_From_3J2T_Full->Add(HistoCollection["eta"]["WWSR1"]["noSyst"][leptonCase]);
  eta_EWKTopSR1_From_3J2T_Full->Add(HistoCollection["eta"]["WZSR1"]["noSyst"][leptonCase]);
  
  if(eta_EWKTopSR1_From_3J2T_Full->Integral()!=0)eta_EWKTopSR1_From_3J2T_Full->Scale(HistoCollection["eta"]["WHFSR1"]["noSyst"][leptonCase]->Integral()/eta_EWKTopSR1_From_3J2T_Full->Integral());
  
  //_Separate SR1 and SR2

  eta_TopSR1_From_3J2T->Add( HistoCollection["eta"]["TopSR1"]["noSyst"][leptonCase]);
  eta_TopSR2_From_3J2T->Add( HistoCollection["eta"]["TopSR2"]["noSyst"][leptonCase]);

  eta_EWKTopSR2_From_3J2T->Add( HistoCollection["eta"]["EWKTopSR2"]["noSyst"][leptonCase]);
  eta_EWKTopSR2_From_3J2T->Add( HistoCollection["eta"]["TopDDSR2"]["noSyst"][leptonCase] );
  eta_EWKTopSR2_From_3J2T->Add(  eta_TopSR2_From_3J2T , -1);
  
  eta_EWKTopSR1_From_3J2T->Add( eta_EWKTopSR2_From_3J2T );
  eta_EWKTopSR1_From_3J2T->Add(HistoCollection["eta"]["WWSR1"]["noSyst"][leptonCase]);
  eta_EWKTopSR1_From_3J2T->Add(HistoCollection["eta"]["WZSR1"]["noSyst"][leptonCase]);

  if(eta_EWKTopSR1_From_3J2T->Integral()!=0)eta_EWKTopSR1_From_3J2T->Scale(HistoCollection["eta"]["WHFSR1"]["noSyst"][leptonCase]->Integral()/eta_EWKTopSR1_From_3J2T->Integral());



  //eta_TopSR1_From_3J2T->Add(HistoCollection["eta"]["TopSR1"]["noSyst"][leptonCase]);
  //eta_TopSR1_From_3J2T->Add(HistoCollection["eta"]["TTBarSR1"]["noSyst"][leptonCase],-1);

  //  eta_TopSR2_From_3J2T->Add( HistoCollection["eta"]["TTBar3J2TSR1"]["noSyst"][leptonCase]);
  //  intt=eta_TopSR2_From_3J2T->Integral();
  //if(intt!=0) eta_TopSR2_From_3J2T->Scale(HistoCollection["eta"]["TTBarSR2"]["noSyst"][leptonCase]->Integral()/intt);
  //eta_TopSR2_From_3J2T->Add(HistoCollection["eta"]["TopSR2"]["noSyst"][leptonCase]);
  //eta_TopSR2_From_3J2T->Add(HistoCollection["eta"]["TTBarSR2"]["noSyst"][leptonCase],-1);
  ...
  eta_TopSR1_From_3J2T->Write();
  eta_TopSR2_From_3J2T->Write();

  eta_EWKTopSR1_From_3J2T->Write();
  eta_EWKTopSR2_From_3J2T->Write();


  eta_TopSR1_From_3J2T_Full->Write();
  eta_TopSR2_From_3J2T_Full->Write();

  eta_EWKTopSR1_From_3J2T_Full->Write();
  eta_EWKTopSR2_From_3J2T_Full->Write();
